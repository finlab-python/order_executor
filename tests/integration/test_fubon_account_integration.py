...
# 導入測試基礎設施
# 導入被測試的模組
from finlab.online.enums import Action, OrderStatus
...
class TestFubonAccountIntegration(IntegrationTestCase, AccountTestMixin):
    ...
    def setUp(self) -> None:
        ...
        try:
            ...
            logging.exception(f"設定 FubonAccount 時發生錯誤: {e}")
            raise
    ...
    def test_get_stocks(self) -> None:
        ...
        for stock_id in test_stocks:
            if stock_id in stocks:
                ...
                logging.info(
                    f"{stock_id}: 收盤價={stock.close}, 買價={stock.bid_price}, 賣價={stock.ask_price}"
                )

    def test_create_and_cancel_order(self) -> None:
        """測試建立和取消委託單"""
        ...
        if order_id:
            ...
            if order_id in orders_after_cancel:
                self.assertEqual(
                    orders_after_cancel[order_id].status, OrderStatus.CANCEL
                )
        else:
            self.skipTest("無法建立委託單，跳過測試")

    def test_create_odd_lot_order(self) -> None:
        """測試零股委託"""
    ...
    def test_update_order_price(self) -> None:
        """測試更新委託單價格"""
        test_data = COMMON_TEST_SCENARIOS["BUY_STOCK_NORMAL"].copy()
    ...
    def test_update_order_quantity(self) -> None:
        """測試更新委託單數量"""
        test_data = COMMON_TEST_SCENARIOS["BUY_STOCK_NORMAL"].copy()
        test_data["price"] = 32.0
        test_data["stock_id"] = "0056"  # 元大高股息
    ...
    def test_update_order_price_and_quantity(self) -> None:
        """測試同時更新委託單價格和數量"""
        test_data = COMMON_TEST_SCENARIOS["BUY_STOCK_NORMAL"].copy()
        test_data["price"] = 32.0
        test_data["stock_id"] = "0056"  # 元大高股息
        ...
        if order_id:
            ...
            new_price = 32.5
            new_quantity = 1
            self.fubon_account.update_order(
                order_id, price=new_price, quantity=new_quantity
            )
            logging.info(
                f"更新委託單 {order_id} 價格為 {new_price}，數量為 {new_quantity} 張"
            )
    ...
    def test_update_odd_lot_order_quantity(self) -> None:
        """測試更新零股委託單數量"""
        test_data = COMMON_TEST_SCENARIOS["BUY_ODD_LOT"].copy()
        test_data["price"] = 30.0  # 使用低於市價的價格避免成交
    ...
    def test_account_balance_consistency(self) -> None:
        """測試帳戶餘額一致性"""
        cash = self.fubon_account.get_cash()
        settlement = self.fubon_account.get_settlement()
        total_balance = self.fubon_account.get_total_balance()
...
class TestFubonAccountOrderExecutor(IntegrationTestCase):
    ...
    def test_order_executor_basic(self) -> None:
    ...
    def test_position_sync(self) -> None:
        """測試持倉同步"""
        # 獲取當前持倉
    ...
    def _test_order_executor_account(self, odd_lot: bool = False) -> None:
        ...
        oe = OrderExecutor(
            Position({sid1: q_sid1, sid2: q_sid2}), account=self.fubon_account
        )
    ...
    def _test_update_price(self, odd_lot: bool = False) -> None:
        ...
        logging.info(
            f"測試 OrderExecutor 價格更新: {sid1}={q_sid1} (odd_lot={odd_lot})"
        )
    ...
    def _check_order_executor(
        self, oe: OrderExecutor, **args_for_creating_orders: Any
    ) -> None:
        ...
        for o in orders.values():
            ...
            sid = o.stock_id
            expected_action = (
                Action.BUY if stock_orders[sid]["quantity"] > 0 else Action.SELL
            )
