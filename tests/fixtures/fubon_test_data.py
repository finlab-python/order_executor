...
from finlab.online.enums import Action, OrderCondition
...
class FubonTestData:
    ...
    @classmethod
    def get_test_position_data(cls) -> list[dict[str, Any]]:
        """獲取測試用持倉數據"""
        return [
            {
                "stock_id": "2330",
    ...
    @classmethod
    def get_test_stock_quotes(cls) -> dict[str, dict[str, Any]]:
        """獲取測試用股票報價"""
        return {
            stock_id: {
                "stock_id": stock_id,
    ...
    @classmethod
    def get_error_scenarios(cls) -> dict[str, dict[str, Any]]:
        """獲取錯誤情境測試數據"""
        return {
            "INVALID_STOCK_ID": {
                "stock_id": "INVALID",
    ...
    @classmethod
    def get_edge_cases(cls) -> dict[str, dict[str, Any]]:
        """獲取邊緣案例測試數據"""
        return {
            "ZERO_QUANTITY": {"quantity": 0, "expected_error": ValueError},
            "NEGATIVE_QUANTITY": {"quantity": -1, "expected_error": ValueError},
            "EMPTY_STOCK_ID": {"stock_id": "", "expected_error": ValueError},
            "NONE_PRICE": {"price": None, "market_order": True},
            "ODD_LOT_DECIMAL": {
