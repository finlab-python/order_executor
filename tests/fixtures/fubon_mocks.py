...
from unittest.mock import Mock, _patch, _patch_dict, patch

from ..utils.mock_helpers import MockBuilder, create_mock_result

if TYPE_CHECKING:
    from finlab.online.brokers.fubon import FubonAccount
...
class FubonSDKMock:
    ...
    def setup_sdk_structure(self) -> None:
        """設置 SDK 基本結構"""
        # 基本方法
        self.sdk.login = Mock()
    ...
    def setup_default_behaviors(self) -> None:
        """設置默認行為"""
        # 登入成功
        account_mock = Mock()
    ...
    def setup_stock_api_defaults(self) -> None:
        ...
        self.sdk.stock.get_order_results.return_value = create_mock_result(
            is_success=True, data=orders_data
        )
        ...
        # modify_quantity 成功
        self.sdk.stock.modify_quantity.return_value = create_mock_result(
            is_success=True
        )
    ...
    def setup_accounting_api_defaults(self) -> None:
        ...
        self.sdk.accounting.bank_remain.return_value = create_mock_result(
            is_success=True, data=balance_data
        )

        # unrealized_gains_and_loses 成功
        unrealized_data = [
            MockBuilder.unrealized_data(stock_no="2330", today_qty=2000),
            MockBuilder.unrealized_data(
                stock_no="2881", today_qty=1000, order_type="Margin"
            ),
        ]
        self.sdk.accounting.unrealized_gains_and_loses.return_value = (
            create_mock_result(is_success=True, data=unrealized_data)
        )
        ...
        self.sdk.accounting.query_settlement.return_value = create_mock_result(
            is_success=True, data=settlement_data
        )

    def setup_marketdata_api_defaults(self) -> None:
    ...
    def set_place_order_failure(self, error_message: str = "下單失敗") -> None:
        """設置下單失敗情境"""
        self.sdk.stock.place_order.return_value = create_mock_result(
            is_success=False, message=error_message
        )

    def set_bank_remain_failure(self, error_message: str = "查詢餘額失敗") -> None:
        """設置餘額查詢失敗情境"""
        self.sdk.accounting.bank_remain.return_value = create_mock_result(
            is_success=False, message=error_message
        )
    ...
    def set_empty_positions(self) -> None:
        """設置空持倉情境"""
        self.sdk.accounting.unrealized_gains_and_loses.return_value = (
            create_mock_result(is_success=True, data=[])
        )

    def set_no_orders(self) -> None:
        """設置無委託單情境"""
        self.sdk.stock.get_order_results.return_value = create_mock_result(
            is_success=True, data=[]
        )

    def set_quote_failure(self) -> None:
        """設置報價查詢失敗情境"""
        self.sdk.marketdata.rest_client.stock.intraday.quote.return_value = None

    def set_login_failure(self, error_message: str = "登入失敗") -> None:
        """設置登入失敗情境"""
...
class FubonAccountMockHelper:
    ...
    @staticmethod
    def create_mock_account_with_sdk(sdk_mock: Mock | None = None) -> FubonAccount:
        """創建帶有 mock SDK 的帳戶實例"""
        if sdk_mock is None:
