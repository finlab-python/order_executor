...
# 導入測試基礎設施
from fubon_neo.constant import BSAction, OrderType

# 導入被測試的模組
from finlab.online.enums import Action, OrderCondition, OrderStatus

from ..fixtures.fubon_mocks import FubonAccountMockHelper, FubonSDKMock
from ..utils.mock_helpers import MockBuilder
from ..utils.test_base import AccountTestMixin, MockTestCase
...
class TestFubonAccountUnit(MockTestCase, AccountTestMixin):
    ...
    def test_init_success(self) -> None:
        """測試帳戶初始化成功"""
        with (
            FubonAccountMockHelper.patch_environment(),
            FubonAccountMockHelper.patch_fubon_sdk() as mock_sdk_class,
        ):
    ...
    def test_init_login_failure(self) -> None:
        """測試登入失敗"""
        with (
            FubonAccountMockHelper.patch_environment(),
            FubonAccountMockHelper.patch_fubon_sdk() as mock_sdk_class,
...
class TestFubonAccountDataParsing(MockTestCase):
    ...
    def test_parse_order_status_all_codes(self) -> None:
        """測試所有狀態碼的解析"""
        test_cases = [
    ...
    def test_parse_order_status_partial_fill_detection(self) -> None:
        """測試部分成交的檢測邏輯"""
        mock_order = Mock()
    ...
    def test_parse_quantities(self) -> None:
        """測試數量解析"""
        mock_order = Mock()
    ...
    def test_parse_order_time_standard_format(self) -> None:
        """測試標準時間格式解析"""
        mock_order = Mock()
    ...
    def test_parse_order_time_without_microseconds(self) -> None:
        """測試無毫秒的時間格式解析"""
        mock_order = Mock()
        mock_order.date = "2023/10/12"
    ...
    def test_map_order_action(self) -> None:
        """測試買賣別映射"""
        test_cases = [(BSAction.Buy, Action.BUY), (BSAction.Sell, Action.SELL)]
    ...
    def test_map_order_action_string_format(self) -> None:
        """測試字串格式的買賣別映射"""
        test_cases = [
            ("B", Action.BUY),
            ("BUY", Action.BUY),
    ...
    def test_map_order_action_invalid(self) -> None:
        """測試無效買賣別的處理"""
        mock_order = Mock()
    ...
    def test_map_order_condition(self) -> None:
        """測試委託條件映射"""
        test_cases = [
    ...
    def test_map_order_condition_string_format(self) -> None:
        """測試字串格式的委託條件映射"""
        test_cases = [
...
class TestFubonAccountStockDataExtraction(MockTestCase, AccountTestMixin):
    ...
    def test_extract_price_data_from_dict(self) -> None:
        """測試從字典格式提取價格數據"""
        quote_dict = {
            "symbol": "2330",
            "openPrice": 580.0,
    ...
    def test_extract_price_data_from_object(self) -> None:
        """測試從物件格式提取價格數據"""
        quote_obj = Mock()
    ...
    def test_extract_bid_ask_data_from_dict(self) -> None:
        """測試從字典格式提取委買委賣數據"""
        quote_dict = {
    ...
    def test_extract_bid_ask_data_empty(self) -> None:
        """測試空的委買委賣數據處理"""
    ...
    def test_create_finlab_stock_success(self) -> None:
    ...
    def test_create_finlab_stock_error_handling(self) -> None:
        """測試創建 Stock 物件時的錯誤處理"""
        # 模擬無效的 quote 數據
...
class TestFubonAccountErrorHandling(MockTestCase):
    ...
    def test_get_cash_exception_handling(self) -> None:
        """測試 get_cash 方法的異常處理"""
        # 設置 SDK 拋出異常
    ...
    def test_get_orders_exception_handling(self) -> None:
...
class TestFubonAccountCreateFinlabOrder(MockTestCase, AccountTestMixin):
    ...
    def test_create_finlab_order_complete(self) -> None:
