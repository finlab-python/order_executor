...
def check_order_executor(
    self: TestSinopacAccount, oe: OrderExecutor, **args_for_creating_orders: Any
) -> None:
...
def f_test_account(self: TestSinopacAccount, fa: Any, odd_lot: bool = False) -> None:
...
def f_test_update_price(
    self: TestSinopacAccount, fa: Any, odd_lot: bool = False
) -> None:
    sid1 = "6016"
    if odd_lot:
        q6016 = 0.1
...
class TestSinopacAccount(unittest.TestCase):
    ...
    def test_sinopac_get_total_balance(self) -> None:
        total_balance = self.sinopac_account.get_total_balance()
        assert total_balance >= 0

    def test_sinopac_order(self) -> None:
        acc = self.sinopac_account
        f_test_account(self, acc, odd_lot=False)

    def test_sinopac_order_odd_lot(self) -> None:
        acc = self.sinopac_account
        f_test_account(self, acc, odd_lot=True)

    def test_sinopac_update_price(self) -> None:
        acc = self.sinopac_account
        f_test_update_price(self, acc, odd_lot=False)

    def test_sinopac_update_price_odd_lot(self) -> None:
        acc = self.sinopac_account
        f_test_update_price(self, acc, odd_lot=True)

    def test_fugle_get_total_balance(self) -> None:
        total_balance = self.fugle_account.get_total_balance()
        assert total_balance >= 0

    def test_fugle_order(self) -> None:
        acc = self.fugle_account
        f_test_account(self, acc, odd_lot=False)

    def test_fugle_order_odd_lot(self) -> None:
        acc = self.fugle_account
        f_test_account(self, acc, odd_lot=True)

    def test_fugle_update_price(self) -> None:
        acc = self.fugle_account
        f_test_update_price(self, acc, odd_lot=False)

    def test_fugle_update_price_odd_lot(self) -> None:
        acc = self.fugle_account
        f_test_update_price(self, acc, odd_lot=True)
    ...
    def test_from_weight(self) -> None:
...
class CalculatePriceWithExtraBidTest(unittest.TestCase):
    def test_calculate_price_with_extra_bid(self) -> None:
    ...
    def test_extra_bid_and_up_down_limit(self) -> None:
        from finlab.online.order_executor import calculate_price_with_extra_bid
...
def check_action_and_position(report: Any) -> None:
