...
class StrategySelector(object):
  ...
  def __init__(self, strategies):
    ...
    with self.out:
      display(widgets.VBox([widgets.HTML('<h2>策略金額</h2>'), form, self.strategy_out]))
  ...
  def update_strategy(self):
    ...
    for i, (s, a) in enumerate(self.strategy_allocation):
      ...
      boxes.append(widgets.HBox([cancel_btn, widgets.Label(value=f'策略名稱：{s}，部位：{a} 元')]))
  ...
  def cancel_strategy(self, i):
    self.strategy_allocation = self.strategy_allocation[:i] + self.strategy_allocation[i+1:]
...
class OrderPanel():
  ...
  def display_position(self, edit=True):
    ...
    stocks = self.oe.account.get_stocks([o['stock_id'] for o in delta.position])
    ...
    for i, o in enumerate(delta.position):
      sid = o['stock_id']
      org_quantity = self.get_quantity(current_position.position, sid, o['order_condition'])
      target_quantity = self.get_quantity(target_position.position, sid, o['order_condition'])
      strategies_with_sid = ", ".join([sname for sname, sids in self.strategy_stocks.items() if sid in sids])

      grid[i, 0] = widgets.Label(value=sid)
      grid[i, 1] = widgets.Label(value=f'原始張數: {org_quantity}')
      grid[i, 2] = widgets.HBox([widgets.Label(value=f'目標張數:'), widgets.FloatText(value=target_quantity, layout=widgets.Layout(width='60px'))])
  ...
  def display_active_order(self):
    ...
    active_orders = [o for oid, o in orders.items() if o.status not in [enums.OrderStatus.CANCEL, enums.OrderStatus.FILLED]]
    ...
    def update_price_btn_func(btn):
      ...
      stock = self.oe.account.get_stocks([order.stock_id])[order.stock_id]
    ...
    def buy_at_market_price_btn_func(btn):
      ...
      if new_quantity >= 1:
        self.oe.account.create_order(order.action, order.stock_id, 1, order_cond=order.order_condition, market_order=True)
        new_quantity -= 1

      if new_quantity > 0:
        self.oe.account.create_order(order.action, order.stock_id, new_quantity, order_cond=order.order_condition)
    ...
    for i, order in enumerate(active_orders):
      ...
      limit_order_update_price_btn.on_click(lambda btn: update_price_btn_func(btn))

      buy_at_market_price_btn = widgets.Button(description='市價買一張')
      buy_at_market_price_btn.oid = order.order_id
      buy_at_market_price_btn.on_click(lambda btn: buy_at_market_price_btn_func(btn))


      grid[i, 0] = widgets.HBox([
        name_label,
        widgets.Label(value=s, layout=widgets.Layout(width='300px')),
        widgets.Label(value=f"ID: {order.order_id}", layout=widgets.Layout(width='300px')),
        cancel_order_btn,
        limit_order_update_price_btn,
        buy_at_market_price_btn
      ])

    keys = ['stock_id', 'action', 'price', 'quantity', 'filled_quantity', 'status', 'order_condition']
