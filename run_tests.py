...
def run_unit_tests(
    test_filter: str | None = None, disable_warnings: bool = True
) -> bool:
    """運行單元測試"""
...
def run_integration_tests(
    test_filter: str | None = None, disable_warnings: bool = False
) -> bool:
    """運行整合測試"""
...
def run_fubon_tests(test_type: str = "all") -> bool:
    """運行富邦相關測試"""
    if test_type == "unit":
...
def check_environment() -> bool:
    ...
    # 檢查富邦憑證
    fubon_vars = ["FUBON_NATIONAL_ID", "FUBON_ACCOUNT_PASS", "FUBON_CERT_PATH"]
...
def main() -> None:
    ...
    parser.add_argument(
        "--fubon", choices=["unit", "integration", "all"], help="運行富邦帳戶測試"
    )
    ...
    # 運行所有測試
        ...
        results.append(
            run_integration_tests(disable_warnings=False)
        )  # 整合測試顯示日誌
